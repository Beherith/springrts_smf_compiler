...
'''	char magic[16];      ///< "spring map file\0"
	...
	float minHeight;     ///< Height value that 0 in the heightmap corresponds tonie 
...
def compileSMF(myargs):
	...
	if myargs.linux:
		cmd = 'CompressonatorCLI -fd DXT1 -RefineSteps 2 -miplevels 9 %s temp/minimap.dds' % ( 	minimapfilename)
		print_flushed (cmd)
		os.system(cmd)
	else:
	...
	if myargs.linux:
		basecmd = 'CompressonatorCLI -fd DXT1 -RefineSteps 2 -miplevels 4 temp/temp%i.%s temp/temp%i.dds'
