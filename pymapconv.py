...
def pythonEncodeDXT1(data): #color bounding box algorithm, this is absolutely trashy and slow and only exists because im too lazy to include a linux compressor
...
def compileSMF(myargs):
	...
	if intex.mode=='RGB':
		...
		print 'Texture image %s is RGBA, thus has an alpha channel. Make absolutely sure that you need this or else consider removing the alpha, as this can cause undesired artefacts!' %(myargs.intex)
	else:
		print 'Texture image %s is neither RGB nor RGBA, but is %s this may cause issues downstream!'%(myargs.intex,intex.mode)
	#open heightmap:
	heights=[]
	if '.raw' in myargs.heightmap.lower():
		...
		pngheight = png.Reader(filename=myargs.heightmap) #(32, 32, <itertools.imap object at 0x10b7eb0>, {'greyscale': True,'alpha': False, 'interlace': 0, 'bitdepth': 2, 'gamma': 1.0})
	...
	featuretypes=['TreeType0', 'TreeType1', 'TreeType2', 'TreeType3', 'TreeType4', 'TreeType5', 'TreeType6', 'TreeType7', 'TreeType8', 'TreeType9', 'TreeType10', 'TreeType11', 'TreeType12', 'TreeType13', 'TreeType14', 'TreeType15', 'GeoVent']
	featureplacement=[]
		...
		for row in range(featuremap.size[1]):
			for col in range(featuremap.size[0]):
				...
				if pixel[1]==255: #geovent
					featureplacement.append({'name':'Geovent','x':8.0*col+4,'y':0.0,'z':8.0*col+4,'rot':0.0,'scale':1.0})
					print 'Placed Geovent: %s'%(str(featureplacement[-1]))
				elif pixel[1]<216 and pixel[1]>199:
					featureplacement.append({'name':'TreeType%i'%(pixel[1]-200),'x':8.0*col+4,'y':0.0,'z':8.0*col+4,'rot':0.0,'scale':1.0})
				elif pixel[1]!=0:
					print 'Undefined green pixel of value %i at %i x %i in %s. Not placing anything'
				if pixel[0]>0:
					try:
						featureplacement.append({'name':featurelist[255-pixel[0]][0],'x':8.0*col+4,'y':0.0,'z':8.0*col+4,'rot':featuretypes[255-pixel[0]][1],'scale':1.0})
					except IndexError:
						print 'Unable to find a featurename in featurelist for red pixel value %i at %ix%i in featuremap!'%(pixel[0],col,row)
	print 'Placed a total of %i features out of %i feature types, with the following distribution:'%(len(featureplacement),len(featuretypes))
	...
	if myargs.geoventfile:
		...
		if sum(geoventimg.size)>1000:
			print 'Warning: You have specified a very large %s geo vent file, are you sure this is what you desire?'%(str(geoventimg.size))
		for feature in featureplacement:
					...
					for col in range(feature['x']-geoventimg.size[0]/2,feature['z']+geoventimg.size[0]/2,1):
						...
						if sum(geoventimg_pixels[geovent_pixel_x,geovent_pixel_y])!=3*255:
							try:
								...
								print 'Warning: Failed to draw a geovent image pixel onto the main texture at %ix%i'%(col,row)
	...
	if myargs.linux:
		...
		os.system('nvdxt.exe -file temp\\temp*.%s -%s -outsamedir -nmips 4 %s'%(extension,compressionmethod,myargs.nvdxt_options))
	...
	print 'sqrtlen vegmap',math.sqrt(len(vegmap))
	for v in vegmap:
	...
	if verbose:
		print 'len minimapdata',len(minimapdata),'of',MINIMAP_SIZE
		print 'len metalmap',len(metalmap)
	for m in metalmap:
...
class SMFMapDecompiler:
	def __init__(self,filename):
		...
		if verbose:
			print 'mapfeaturesheader=',self.mapfeaturesheader
			print 'maptileheader=',MapTileHeader_struct.unpack_from(self.smffile,self.tilesPtr)
...
if __name__ == "__main__":
	...
	parser.add_argument('-x','--maxheight', help= ' <max height> (required) What altitude in spring the max(0xff for 8 bit images or 0xffff for 16bit images) level of the height map represents', default = 100.0, type=float)
	parser.add_argument('-n','--minheight', help= ' <min height> (required) What altitude in spring the minimum level (0) of the height map represents', default = -50.0, type=float)
	parser.add_argument('-o', '--outfile', help = ' <output mapname.smf> (required) The name of the created map file. Should end in .smf. A tilefile (extension .smt) is also created, this name may contain spaces', default = '', type = str )
	parser.add_argument('-m', '--metalmap', help = '<metalmap.bmp> Metal map to use, red channel is amount of metal. Resized to xsize / 2 by ysize / 2.', type = str )
	parser.add_argument('-t', '--intex', help = '<texturemap.bmp> (required) Input bitmap to use for the map. Sides must be multiple of 1024 long. Xsize and Ysize are determined from this file; xsize = intex width / 8, ysize = height / 8', default = '', type = str )
	parser.add_argument('-a', '--heightmap', help = '<heightmap file> (required) Input heightmap to use for the map, this should be 16 bit greyscale PNG image or a 16bit intel byte order single channel .raw image. Must be xsize*64+1 by ysize*64+1', default = '', type = str )

	parser.add_argument('-g', '--geoventfile', help = '<geovent.bmp> The decal for geothermal vents; appears on the compiled map at each vent. Custom geovent decals should use all white as transparent, clear this if you do not wish to have geovents drawn.', default = 'geovent.bmp', type = str )
	parser.add_argument('-y', '--typemap', help =  '<typemap.bmp> Type map to use, uses the red channel to decide terrain type. types are defined in the .smd, if this argument is skipped the entire map will TERRAINTYPE0',  type = str )
	...
	parser.add_argument('-k','--featureplacement',help= '<featureplacement.lua> A feature placement text file defining the placement of each feature. (Default: fp.txt). See README.txt for details. The default format specifies it to have each line look like this: \n { name = \'agorm_talltree6\', x = 224, z = 3616, rot = "0" , scale = 1.0} \n the [scale] argument currently does nothing in the engine. ', type = str)
	parser.add_argument('-j','--featurelist',help= '<feature_list_file.txt> (required if featuremap image is specified) A file with the name of one feature on each line. Specifying a number from 32767 to -32768 next to the feature name will tell mapconv how much to rotate the feature. specifying -1 will rotate it randomly.', type=str)
	parser.add_argument('-f','--featuremap',help= ' <featuremap.bmp> Feature placement image, xsize by ysize. Green 255 pixels are geo vents, blue is grass, green 201-215 are engine default trees, red 255-0 each correspond to a line in --featurelist', type=str)
	parser.add_argument('-p','--minimap',help= ' <minimap.bmp> If specified, will override generating a minimap from the texture file (intex) with the specified file', type=str)
	parser.add_argument('-r','--grassmap',help= ' <grassmap.bmp> If specified, will override the grass specified in the featuremap. Expects an xsize/4 x ysize/4 sized bitmap, all values that are not 0 will result in grass', type=str)

	#parser.add_argument('-s', '--justsmf', help = 'Just create smf file, dont make tile file (for quick recompilations)', default = 0, type=int)
	parser.add_argument('-u', '--linux', help = 'Check this if you are running linux and wish to use imagemagicks convert utility instead of nvdxt.exe',default=False, action='store_true')
	...
	a = argparseui.ArgparseUi(parser, left_label_alignment=True, use_scrollbars=True, use_save_load_button=True,ok_button_handler=okbuttonhandler, window_title = "Spring Map Format (SMF) compiler and decompiler")
	a.show()
	app.exec_()
